...
//        Move move = current.getMove();
        ...
        if (pieceTwo == nullptr) {std::cout << "No Piece at destination spot" << std::endl;}
        ...
        if (pieceTwo != nullptr and pieceTwo->getIswhite() == pieceOne->getIswhite()) {std::cout << "The spot you want to move to is blocked by a piece of your color. " << pieceTwo->getIswhite() << "" << pieceOne->getIswhite() << std::endl; return false;}
...
//        for (int i = 0; i<8; i++) {
//            position2 =
//            if (Maketurn(current, other, move) == false) {checkMove = false;}
//        }
