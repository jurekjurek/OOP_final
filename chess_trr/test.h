...
//                if (!isAttackingSpot(current, kingPos.shiftPos(i, j)) or this->Board.getPiece(kingPos.shiftPos(i, j))->getIswhite() == other.getIswhite() ) {return false;}
    ...
    void Game() {
        ...
        while (state == ACTIVE or state == CHECK) {
            ...
            Player current;
            Player other;
            if (this->Whoseturn == true) {current = Board.getPlayerWhite(); other = Board.getPlayerBlack(); cout << "Whites move" << endl;}
            else {other = Board.getPlayerWhite(); current = Board.getPlayerBlack(); cout << "Blacks move" << endl;}
